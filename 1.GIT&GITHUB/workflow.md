...
- this job concurrency use when so many job in queue, we discard old ones and run only new job 
1. go to actions tab
2. click on workflow name
3. click on job name
4. click on concurrency on left side in editor
5. add concurrency name
6. add concurrency strategy
7. add concurrency cancel in-progress
8. add concurrency cancel existing
- add below code in existing workflow yml file
```
concurrency:
